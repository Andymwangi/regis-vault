...
export default function FileManagerPage() {
  ...
  const [departments, setDepartments] = useState<Department[]>([]); // Initialize as empty array
  ...
  const fetchDepartments = async () => {
    try {
      ...
      setDepartments(data.departments || []); // Add fallback to empty array
    } catch (error) {
  ...
  const fetchFiles = async () => {
    try {
      ...
      const data = await response.json();
      setFiles(data.files || []); // Add fallback to empty array
      setPagination(data.pagination || { total: 0, pages: 1, current: 1 }); // Add fallback
    } catch (error) {
